...
func main() {
	...
	r.HandleFunc("/users/{id}", h.UpdateEmployee).Methods("PUT")
	r.HandleFunc("/users/{id}", h.DeleteEmployee).Methods("DELETE")
	r.HandleFunc("/users", h.FetchAllEmployees).Methods("GET")
